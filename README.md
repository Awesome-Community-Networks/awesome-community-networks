# Awesome Community Networks 🌍

A curated list of community networks, mesh technologies, and grassroots connectivity efforts around the world.

[![Awesome](https://awesome.re/badge.svg)](https://awesome.re)

---

## Contents

- [Networks by Continent](#networks-by-continent)
- [Tools & Software](#tools--software)
- [Hardware Projects](#hardware-projects)
- [Governance & Policy](#governance--policy)
- [Learning Resources](#learning-resources)
- [Events & Conferences](#events--conferences)
- [Communities & Mailing Lists](#communities--mailing-lists)

---

## Networks by Continent
  
### South America
- [Altermundi - Argentina - Quintana](https://conectividad.altermundi.net/) - Grassroots connectivity and LibreRouter development
- [Carc - Argentina](https://web.archive.org/web/20241116083113/https://carc.libre.org.ar/) - Argentine summit of community networks
- [Coolab - Brazil](https://coolab.org/) - Cooperative for free internet and community connectivity
- [ElValleReinicia - Argentina - Las Calles, Córdoba](https://radioslibres.net/conoce-a-la-red-comunitaria-el-valle-reinicia/) - Community network deployed with LibreRouter
- [MonteNet - Argentina - Molinari](https://montelibre.net/) - Community network in Molinari, Córdoba
- [NonoLibre - Argentina - Nono (Traslasierra Valley)](https://programafrida.net/archivos/project/red-traslasierra) - Community network in the Traslasierra Valley
- [Rhizomatica - Mexico](https://rhizomatica.org/) - Community cellular networks for indigenous peoples
- [Rede Mocambos - Brazil](https://mocambos.net/) - Quilombola community network connecting rural settlements

### Europe
- [Antennine - Italy](https://antennine.org/) - Community wireless network in Palermo, Sicily
- [AWMN - Greece](https://www.awmn.net/) - Athens Wireless Metropolitan Network
- [Freifunk - Germany](https://freifunk.net/) - Nationwide initiative for free and open mesh networks
- [FunkFeuer - Austria](https://funkfeuer.at/) - Free experimental community network with nodes across Austria
- [Guifi.net - Spain](https://fundacio.guifi.net/) - One of the largest community networks in the world, with thousands of nodes
- [Ninux - Italy](http://wiki.ninux.org/) - Community wireless network in Rome and across Italy
- [Sarantaporo.gr - Greece](https://sarantaporo.gr/) - Community wireless network connecting mountain villages in Thessaly

### Africa
- [Zenzeleni - South Africa](https://zenzeleni.net/) - Community network in Mankosi serving rural Eastern Cape
- [TunapandaNET - Kenya](https://www.apc.org/en/users/tunapanda-institute) - Community network providing connectivity and digital skills in Nairobi

### Asia
- [Common Room Networks Foundation - Indonesia](https://commonroom.info/) - Community networks empowering indigenous and rural villages in West Java
- [Digital Empowerment Foundation - India](https://defindia.org/) - Digital literacy and community networks across rural India


### North America
- [NYC Mesh - USA - New York City](https://nycmesh.net/) - Community-owned mesh network ending the digital divide in NYC
- [People's Open Network - USA - Oakland](https://github.com/sudomesh) - Community wireless mesh network in the Bay Area
- [Rhizomatica - Mexico](https://rhizomatica.org/) - Community cellular networks for indigenous peoples
- [Seattle Community Network - USA - Seattle](https://seattlecommunitynetwork.org/) - Community-run 4G LTE network for fair internet access
- [TIC A.C. - Mexico](https://ticac.org/) - Indigenous community-owned cellular and mesh network
- [Vancouver Community Network - Canada](https://www2.vcn.bc.ca/) - Free community network providing internet access and digital equity

---

## Tools & Software

- [OpenWRT](https://openwrt.org/) - Linux operating system for embedded devices, the foundation of most community network firmware
- [LibreMesh](https://libremesh.org/) - Open-source firmware for community mesh networking on OpenWRT
- [gluon](https://gluon.readthedocs.io/en/latest/) - Firmware framework for Freifunk and other wireless community networks
- [Retroshare](https://retroshare.cc/) - Secure decentralized communication and file-sharing platform
- [Qaul](https://github.com/qaul/qaul.net) - Internet-independent mesh communication app
- [Yggdrasil](https://yggdrasil-network.github.io/) - Early-stage fully end-to-end encrypted IPv6 mesh network
- [Meshenger](https://github.com/dakhnod/Meshenger) - P2P voice and video calls over local WiFi mesh
- [Reticulum](https://reticulum.network/) - Cryptographic networking stack for building secure networks over any medium
- [Meshtastic](https://meshtastic.org/) - Open-source LoRa mesh communicator for off-grid text messaging
- [batman-adv](https://www.open-mesh.org/) - Layer-2 mesh routing protocol used by Freifunk, LibreMesh, gluon
- [OLSRd](https://www.olsr.org/) - Optimized Link State Routing protocol for mesh networks
- [BMX7](https://github.com/bmx-routing/bmx7) - Securely Entrusted Mesh Routing Protocol for Linux
- [CJDNS](https://github.com/cjdelisle/cjdns) - Encrypted IPv6 mesh network using public-key cryptography
- [FIPS](https://github.com/jmcorgan/fips) - Self-organizing encrypted mesh network built on Nostr identities
- [nostr-vpn](https://github.com/mmalmi/nostr-vpn) - Tailscale-style private mesh VPN using FIPS data plane
- [TollGate](https://github.com/OpenTollGate/tollgate-module-basic-go) - Bearer instruments for transport
- [wifistr](https://github.com/hzrd149/wifistr) - Open WiFi map powered by Nostr


---

## Hardware Projects

- [LibreRouter](https://gitlab.com/librerouter) - Open-source hardware router specifically designed for community networks with dual-band support and LibreMesh integration.
- [Mesh Potato](https://villagetelco.org/mesh-potato/) - Low-cost mesh telephony device combining WiFi mesh networking with VoIP capabilities for rural areas.
- [Meshbox](https://github.com/libremesh/lime-packages) - Plug-and-play mesh network router hardware running LibreMesh firmware.
- [OpenWrt-One](https://openwrt.org/toh/openwrt/one) - Reference hardware platform designed specifically for OpenWrt development and deployment.

---

## Governance & Policy
- [Community Network Manual by APC](https://www.apc.org/en/pubs/community-networks-manual) - Comprehensive guide by the Association for Progressive Communications on building and sustaining community networks.
- [Community Networks as a public policy](https://tau.org.ar/wp-content/uploads/2023/11/NodoTAU_AlterMundi_-A-public-policy-in-motion-ENG-1.pdf) - Research paper by Altermundi on community networks as public policy initiatives in Argentina.
- [IGF Dynamic Coalition on Community Connectivity](https://comconnectivity.org/) - Internet Governance Forum initiative promoting community-driven approaches to connectivity and digital inclusion.

---

## Learning Resources

- [APC CN Learning Repository](https://cnlearning.apc.org/) - Global hub with manuals, training programs, and research for community networks
- [Altermundi's Community Network Curriculum](https://conectividad.altermundi.net/documentacion/) - Educational materials and training resources for building and managing community networks.
- [Common Room Library](https://commonroom.info/category/library/) - Learning materials and documentation from the Indonesian School for Community Networking
- [Creating wireless community networks with LibreRouter, LibreMesh and other tools](https://www.earthdefenderstoolkit.com/one-pager/guide-librerouter-and-libremesh/) - Practical guide for setting up community wireless networks using open-source tools.
- [Freifunk Blog](https://blog.freifunk.net) - News, updates, and technical articles from the Freifunk community network movement in Germany.
- [Internet Society CN DIY Toolkit](https://www.internetsociety.org/resources/community-network-diy-toolkit/understand-community-network-examples/) - Comprehensive guide to planning and deploying community networks
- [Rural ICT Camp](https://ruralictcamp.commonroom.info/) - International knowledge-sharing platform for rural connectivity and digital skills
- [Wireless Networking in the Developing World](https://wndw.net/) - Free book and resource covering the practical aspects of designing and building wireless networks in underserved areas.

---

## Events & Conferences

- [Radical Networks](https://radicalnetworks.org/) - Conference on creative, critical, and alternative network infrastructures
- [Summit on Community Networks in Africa](https://www.internetsociety.org/events/summit-community-networks-africa/) - Annual gathering organized by the Internet Society to advance community connectivity across Africa.
- [Wireless BattleMesh](https://battlemesh.org/) - Annual event where mesh networking developers and community network builders test and compare routing protocols and technologies.

---

## Communities & Mailing Lists

- [BattleMesh Matrix Room](https://matrix.to/#/#battlemesh:matrix.org) - Real-time chat room for mesh networking enthusiasts and developers on the Matrix protocol.
- [LibreMesh Matrix channel](https://matrix.to/#/#libremesh-dev:matrix.guifi.net) - Official community discussion platform for LibreMesh firmware users and developers.
- [LibreMesh Mailing List](https://www.autistici.org/mailman/listinfo/libremesh) - Official mailing list for LibreMesh community announcements, support and development discussion.
